...
function confirmReset() {
  const confirmed = window.confirm(
    "Reset the entire catalog to defaults? This cannot be undone.",
  );
  if (!confirmed) return;
...
<template>
  <div class="min-h-screen bg-slate-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div class="mt-1 flex gap-2">
            ...
            <button
              class="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors"
              @click="confirmReset"
            >
              Reset Catalog
            </button>
      ...
      <div
        v-if="notice"
        class="mb-4 rounded-lg border px-4 py-3 text-sm flex items-center justify-between"
        :class="
          notice.ok
      ...
      <!-- Add / Edit Form -->
      <div
        v-if="showAddForm || editingProductId"
      ...
      >
        ...
        <form
          ...
          @submit.prevent="editingProductId ? submitEdit() : submitAdd()"
        >
          <div>
            <label
            ...
            <input
              ...
              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50">
              <th
                class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-slate-500"
              >
                ID
              </th>
              <th
            ...
            >
              ...
              <td class="px-5 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button
                    class="rounded-md border border-slate-300 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 transition-colors"
                    @click="openEditForm(product)"
                  >
                    Edit
                  </button>
                  <button
                    class="rounded-md border border-rose-200 px-2.5 py-1 text-xs font-medium text-rose-600 hover:bg-rose-50 transition-colors"
